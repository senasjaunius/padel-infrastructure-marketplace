export default async function handler(req,res){
 ...
 try{
  ...
  const requestPayload={
    seller_id:seller?.id||null,company,email,country,city:String(b.city||'').trim(),
    court_count:Math.max(1,Number(b.count||1)),year_installed:b.year?Number(b.year):null,
    manufacturer:String(b.manufacturer||'').trim()||null,model:String(b.model||'').trim()||null,
    glass_thickness_mm:b.glass_thickness_mm?Number(b.glass_thickness_mm):null,
    condition:String(b.condition||'').trim()||null,turf_condition:String(b.turf_condition||'').trim()||null,
    lighting_included:b.lighting_included==='true',asking_price_eur:b.asking_price_eur?Number(b.asking_price_eur):null,
    photo_urls:String(b.photo_urls||'').trim()||null,details:String(b.details||'').trim()||null
  };
