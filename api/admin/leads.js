export default async function handler(req,res){
 ...
 try{
  const [buyers,services,sellers,matches]=await Promise.all([
  ...
  return res.status(200).json({buyers:await read(buyers),services:await read(services),sellers:await read(sellers),matches:await read(matches)});
